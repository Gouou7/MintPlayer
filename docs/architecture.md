...
Use separate mounted and visible states for the embedded presentation. Mount one fixed-size composited shell offscreen before animating the complete surface. Keep the native toolbar for the normal main interface and windowed lyrics presentation. When embedded lyrics enter full screen, a focused `NSWindow` bridge temporarily hides the toolbar without removing it, extends the lyrics surface through the titlebar area, and exposes a lyrics-owned immersive control layer. The bridge restores every captured window and toolbar property when full screen or lyrics presentation ends.
