...
- 支持主窗口内嵌或独立窗口两种歌词展示方式，提供同步本地 `.lrc` 歌词、平滑滚动、非当前行可选模糊，以及独立窗口尺寸记忆。
- 支持播放队列、随机播放、循环、上一曲/下一曲、播放状态恢复和 Dock 菜单控制。
- 支持喜欢、屏蔽歌曲、有效播放次数统计和资料库状态持久化。
- 通过 Now Playing 和远程媒体控制与系统媒体功能集成。
- 设置中可调整主题、语言、歌词打开方式、歌词模糊、资料库文件夹、重新扫描和屏蔽歌曲管理。
...
Mint Player 支持本地 `.lrc` 歌词。歌词会跟随播放进度、高亮当前行，并支持点击歌词跳转播放位置。你可以在设置中选择点击播放器封面后，在主窗口内以沉浸式覆盖层打开歌词，或继续使用原有的独立歌词窗口。

内嵌歌词页面会滑入并覆盖整个资料库界面，同时保留下层当前的侧栏、选择、滚动位置和详情导航状态。可以通过工具栏关闭按钮或 `Esc` 收起。独立歌词窗口仍支持调整大小，并记忆窗口尺寸和位置。非当前歌词行的模糊效果可以单独开启或关闭。
